...
async def scrape_html_size_chart(brand_name, target_url):
    ...
    try:
        ...
        try:
            ...
            logger.warning("playwright-stealth is not installed. Running without stealth mode. (pip install playwright-stealth)")
            stealth_async = None
    ...
    logger.info(f"[{brand_name}] Starting async Playwright to scrape size chart at {target_url}")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            logger.info(f"[{brand_name}] Searching for a 'Size Guide' or 'Size Chart' button to click...")
            import re
            try:
                # Common elements that contain size guides ("Size Guide", "Size Chart", "Sizing")
                size_button = page.locator("button, a, span, div").filter(has_text=re.compile(r"(size\s*(guide|chart)|sizing)", re.IGNORECASE)).first
                await size_button.wait_for(state="visible", timeout=5000)
                logger.info(f"[{brand_name}] Found sizing button. Clicking to open chart modal...")
                await size_button.click()
                await asyncio.sleep(3) # Give the modal time to open and render the table
            except Exception as e:
                logger.info(f"[{brand_name}] Sizing button not found within 5s (the table may already be visible). Proceeding...")
            ...
            if tables:
                ...
                logger.info(f"[{brand_name}] No standard <table> found. Looking for <div>-based charts (Type B)...")
                chart_divs = await page.query_selector_all("div[class*='size'], div[class*='chart']")
                if chart_divs:
                    logger.info(f"[{brand_name}] Found generic size chart container <div>.")
                    ...
                    logger.warning(f"[{brand_name}] Could not find <table> or size-chart <div> elements.")
